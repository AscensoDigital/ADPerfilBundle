...
class Configuration implements ConfigurationInterface
{
    ...
    public function getConfigTreeBuilder()
    {
        ...
        $rootNode
            ->children()
                ->scalarNode('perfil_class')
                    ->isRequired()
                    ->cannotBeEmpty()
                ->end()
                ->scalarNode('perfil_table_alias')
                    ->isRequired()
                    ->cannotBeEmpty()
                ->end()
                ...
                ->scalarNode('session_name')
                    ->defaultValue('ad_perfil.perfil_id')
                ->end()
                ...
                ->scalarNode('icon_alt')
                    ->isRequired()
                ->end()
                ->integerNode('proveedor_id')
                    ->min(0)    
                    ->defaultValue(0)
                ->end()
                ->scalarNode('valor_true')
                    ->defaultValue('1')
                ->end()
                ->scalarNode('valor_false')
                    ->defaultValue('0')
                ->end()
                ->scalarNode('separador_encabezado')
                    ->defaultValue(false)
                ->end()
                ->arrayNode('navegacion')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->scalarNode('homepage_title')
                            ->isRequired()
                            ->cannotBeEmpty()
                        ->end()
                        ->scalarNode('homepage_subtitle')
                            ->defaultValue('')
                        ->end()
                        ->scalarNode('homepage_route')
                            ...
                            ->cannotBeEmpty()
                        ->end()
                        ->scalarNode('homepage_name')
                            ->defaultValue('Inicio')
                            ->cannotBeEmpty()
                        ->end()
                        ->scalarNode('homepage_icono')
                            ->defaultValue('fa fa-home')
                            ->cannotBeEmpty()
                        ->end()
                        ->scalarNode('homepage_color')
                            ->defaultValue('blanco')
                            ->cannotBeEmpty()
                        ->end()
                    ->end()
